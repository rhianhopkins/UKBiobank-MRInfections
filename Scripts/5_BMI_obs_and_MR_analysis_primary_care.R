...
##Bacterial
#Pneumonia
pneumonia_lr <- glm(pneumonia_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(pneumonia_GP == 1 | control_GP == 1)), family = "binomial")
#Skin infection
bacterial_skininfection_lr <- glm(bacterial_skininfection_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(bacterial_skininfection_GP == 1 | control_GP == 1)), family = "binomial")
#UTI
UTI_lr <- glm(UTI_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(UTI_GP == 1 | control_GP == 1)), family = "binomial")
#Cystitis
cystitis_lr <- glm(cystitis_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(cystitis_GP == 1 | control_GP == 1)), family = "binomial")
#Pyelonephritis
pyelonephritis_lr <- glm(pyelonephritis_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(pyelonephritis_GP == 1 | control_GP == 1)), family = "binomial")

##Viral
#Influenza
influenza_lr <- glm(influenza_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(influenza_GP == 1 | control_GP == 1)), family = "binomial")
#Other respiratory infection
otherrespiratoryinfection_lr <- glm(otherrespiratoryinfection_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(otherrespiratoryinfection_GP == 1 | control_GP == 1)), family = "binomial")
#Lower respiratory infection
lower_resp_lr <- glm(lower_resp_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(lower_resp_GP == 1 | control_GP == 1)), family = "binomial")
#Upper respiratory infection
upper_resp_lr <- glm(upper_resp_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(upper_resp_GP == 1 | control_GP == 1)), family = "binomial")

##Fungal
#Skin infection
fungal_skininfection_lr <- glm(fungal_skininfection_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(fungal_skininfection_GP == 1 | control_GP == 1)), family = "binomial")
#Genital infection
genitalinfection_lr <- glm(genitalinfection_GP ~ bmi + sex + recruit_age, data = (bmi_obs_df %>% filter(genitalinfection_GP == 1 | control_GP == 1)), family = "binomial")
...
Obs_BMI <- Obs_BMI %>%
  mutate(case_when(
    Infection == "Pneumonia" ~ count(bmi_obs_df %>% filter(pneumonia_GP ==1)),
    Infection == "Bacterial skin infection" ~ count(bmi_obs_df %>% filter(bacterial_skininfection_GP ==1)),
    Infection == "UTI" ~ count(bmi_obs_df %>% filter(UTI_GP ==1)),
    Infection == "Cystitis" ~  count(bmi_obs_df %>% filter(cystitis_GP ==1)),
    Infection == "Pyelonephritis" ~  count(bmi_obs_df %>% filter(pyelonephritis_GP ==1)),
    Infection == "Influenza" ~  count(bmi_obs_df %>% filter(influenza_GP ==1)),
    Infection == "Other viral respiratory" ~  count(bmi_obs_df %>% filter(otherrespiratoryinfection_GP ==1)),
    Infection == "Lower respiratory" ~  count(bmi_obs_df %>% filter(lower_resp_GP ==1)),
    Infection == "Upper respiratory" ~  count(bmi_obs_df %>% filter(upper_resp_GP ==1)),
    Infection == "Fungal skin infection" ~  count(bmi_obs_df %>% filter(fungal_skininfection_GP ==1)),
    Infection == "Genital infection" ~  count(bmi_obs_df %>% filter(genitalinfection_GP ==1))
  )
  )
...
# Save table as CSV
write.csv(Obs_BMI, file = paste0("saveables/", "Obs_BMI_results_primary_care_by_type.csv"), row.names=FALSE)
...
##Bacterial
#Pneumonia
pneumonia_mr <- glm(pneumonia_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(pneumonia_GP == 1 | control_GP == 1)), family = "binomial")
#Skin infection
bacterial_skininfection_mr <- glm(bacterial_skininfection_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(bacterial_skininfection_GP == 1 | control_GP == 1)), family = "binomial")
#UTI
UTI_mr <- glm(UTI_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(UTI_GP == 1 | control_GP == 1)), family = "binomial")
#Cystitis
cystitis_mr <- glm(cystitis_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(cystitis_GP == 1 | control_GP == 1)), family = "binomial")
#Pyelonephritis
pyelonephritis_mr <- glm(pyelonephritis_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(pyelonephritis_GP == 1 | control_GP == 1)), family = "binomial")

##Viral
#Influenza
influenza_mr <- glm(influenza_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(influenza_GP == 1 | control_GP == 1)), family = "binomial")
#Other respiratory infection
otherrespiratoryinfection_mr <- glm(otherrespiratoryinfection_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(otherrespiratoryinfection_GP == 1 | control_GP == 1)), family = "binomial")
#Lower respiratory infection
lower_resp_mr <- glm(lower_resp_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(lower_resp_GP == 1 | control_GP == 1)), family = "binomial")
#Upper respiratory infection
upper_resp_mr <- glm(upper_resp_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(upper_resp_GP == 1 | control_GP == 1)), family = "binomial")

##Fungal
#Skin infection
fungal_skininfection_mr <- glm(fungal_skininfection_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(fungal_skininfection_GP == 1 | control_GP == 1)), family = "binomial")
#Genital infection
genitalinfection_mr <- glm(genitalinfection_GP ~ bmi_pred + sex + recruit_age + bmi_resid, data = (bmi_mr_df %>% filter(genitalinfection_GP == 1 | control_GP == 1)), family = "binomial")
...
MR_BMI <- MR_BMI %>%
  mutate(case_when(
    Infection == "Pneumonia" ~ count(bmi_mr_df %>% filter(pneumonia_GP ==1)),
    Infection == "Bacterial skin infection" ~ count(bmi_mr_df %>% filter(bacterial_skininfection_GP ==1)),
    Infection == "UTI" ~ count(bmi_mr_df %>% filter(UTI_GP ==1)),
    Infection == "Cystitis" ~  count(bmi_mr_df %>% filter(cystitis_GP ==1)),
    Infection == "Pyelonephritis" ~  count(bmi_mr_df %>% filter(pyelonephritis_GP ==1)),
    Infection == "Influenza" ~  count(bmi_mr_df %>% filter(influenza_GP ==1)),
    Infection == "Other viral respiratory" ~  count(bmi_mr_df %>% filter(otherrespiratoryinfection_GP ==1)),
    Infection == "Lower respiratory" ~  count(bmi_mr_df %>% filter(lower_resp_GP ==1)),
    Infection == "Upper respiratory" ~  count(bmi_mr_df %>% filter(upper_resp_GP ==1)),
    Infection == "Fungal skin infection" ~  count(bmi_mr_df %>% filter(fungal_skininfection_GP ==1)),
    Infection == "Genital infection" ~  count(bmi_mr_df %>% filter(genitalinfection_GP ==1))
  )
  )
...
# Save table as CSV
write.csv(MR_BMI, file = paste0("saveables/", "MR_BMI_results_primary_care_by_type.csv"), row.names=FALSE)
