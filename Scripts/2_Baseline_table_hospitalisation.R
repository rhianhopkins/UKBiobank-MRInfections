...
infections_df <- infections_df %>% mutate(infection_control = ifelse(control ==1, "Control", "Infection"))
...
all_vars <- c("sex", "femalesex", "assess_age", "bmi", "hba1c", "glucose", "whr", "diabetes_diagnosed", "tdi", "smoking_status", 
              "bacterial_skininfection", "fungal_skininfection", "pneumonia", "influenza", "otherrespiratoryinfection",
              "lower_resp", "upper_resp", "UTI", "cystitis", "pyelonephritis", "genitalinfection")
...
# Save table as CSV
write.csv(tab_all, file = paste0("saveables/", "baseline_table_all.csv"), row.names=FALSE)
...
#Controls
tableone_controls <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(control == TRUE)), factorVars=categorical_vars, test=FALSE)
...
tableone_skin_bact <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(bacterial_skininfection == TRUE)), factorVars=categorical_vars, test=FALSE)
...
tableone_skin_fun <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(fungal_skininfection == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Pneumonia
tableone_pneumo <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(pneumonia == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Influenza
tableone_flu <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(influenza == TRUE)), factorVars=categorical_vars, test=FALSE)
...
tableone_resp <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(otherrespiratoryinfection == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Lower
tableone_lowerresp <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(lower_resp == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Upper
tableone_upperresp <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(upper_resp == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#UTI
tableone_UTI <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(UTI == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Cystitis
tableone_cystitis <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(cystitis == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Pyelonephritis
tableone_pyelonephritis <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(pyelonephritis == TRUE)), factorVars=categorical_vars, test=FALSE)
...
#Genital infection
tableone_genitalinfection <- CreateTableOne(vars=all_vars,data= (infections_df %>% filter(genitalinfection == TRUE)), factorVars=categorical_vars, test=FALSE)
...
# Save table as CSV
write.csv(tab_all_infections, file = paste0("saveables/", "baseline_table_by_type.csv"), row.names=FALSE)
